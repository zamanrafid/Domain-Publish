...
/* ============================================================
   MEDIA EDITING DISABLED
   The old in-browser "add photo/video" feature has been fully
   turned off below (openManageModal is now a no-op, and nothing
   in the UI can trigger it). All portfolio content changes are
   now made directly in this file's code, then redeployed. This
   also wipes any leftover local test data from before.
   ============================================================ */
(function () {
  window.__RZ_OWNER__ = false;
  try { localStorage.removeItem('rz_portfolio_items_v1'); } catch (e) {}
  try { localStorage.removeItem('rz_owner_unlock'); } catch (e) {}
})();
...
(function(){
  ...
  function buildLatestFromPortfolio(catKey, max){
    ...
    order.forEach(function(idx){
      ...
      var first = null;
      for(var _fi=0; _fi<imgs.length; _fi++){ var _c = imgs[_fi]; if(typeof _c === "string" && _c){ first = _c; break; } }
      ...
      if(typeof isDirectVideoUrl === "function" && isDirectVideoUrl(first)){
        ...
        var thumb = (typeof getThumbSrc === "function") ? getThumbSrc(first) : null;
        if(!thumb) return;
        entry = { type: "image", src: thumb };
      } else {
  ...
  var IMAGE_DURATION = 1400;
  var VIDEO_MAX_DURATION = 7000;
  ...
  CAT_ORDER.forEach(function(cat){
    ...
    state.slideEls = state.items.map(function(item, idx){
      ...
      if(item.type === "video"){
        ...
        media.src = optimizeImg(item.src, 900, 55);
      ...
      media.classList.add("lw-media-loading");
      var __clearShimmer = function(){ media.classList.remove("lw-media-loading"); };
      if(item.type === "video"){ media.addEventListener("loadeddata", __clearShimmer); }
      else { media.complete ? __clearShimmer() : media.addEventListener("load", __clearShimmer); }
    ...
    state.thumbEls = state.items.map(function(item, idx){
      ...
      if(item.type === "video"){
        ...
        media = document.createElement("img");
        media.src = optimizeImg(item.src, 160, 50);
        media.alt = "";
        media.loading = "lazy";
        media.decoding = "async";
        btn.appendChild(media);
  ...
  function buildSlideEl(state, cat, item, idx){
    ...
    media.classList.add("lw-media-loading");
    var __clearShimmer2 = function(){ media.classList.remove("lw-media-loading"); };
    if(item.type === "video"){ media.addEventListener("loadeddata", __clearShimmer2); }
    else { media.complete ? __clearShimmer2() : media.addEventListener("load", __clearShimmer2); }
    media.addEventListener("error", function(){ handleBroken(cat, idx); });
    el.appendChild(media);
    if(item.link){
      var linkBtn = document.createElement("a");
  ...
  function preloadSlide(cat, idx, done){
    ...
    if(item.type === "video"){
      ...
      var optimizedSrc = optimizeImg(item.src, 900, 55);
...
/* ============================================================
   MSHOTS AUTO-REFRESH
   s.wordpress.com/mshots generates website screenshots on
   demand ? the first request(s) after a URL is added return a
   small generic "generating thumbnail" placeholder (much
   narrower than the requested width) until the real screenshot
   is ready. Without this, a newly added project thumbnail can
   look stretched/broken (a tiny placeholder blown up to fill
   the frame) until someone manually reloads the page later.
   This watches every mshots <img> (now or added later, e.g.
   lazy-loaded portfolio thumbs), checks the image that actually
   loaded, and ? if it's clearly the small placeholder rather
   than the full-width screenshot ? keeps retrying with a
   cache-buster on a backoff schedule (up to ~5 minutes) until
   the real screenshot comes back, then leaves it alone.
   ============================================================ */
(function () {
  var RETRY_DELAYS = [6000, 12000, 20000, 30000, 45000, 60000, 90000, 120000, 180000];
  var MIN_REAL_WIDTH = 700; // requested width is 1400; mshots placeholder is far narrower
  var tracked = typeof WeakMap !== "undefined" ? new WeakMap() : null;

  function isPlaceholder(img) {
    return !!(img.naturalWidth && img.naturalWidth < MIN_REAL_WIDTH);
  }

  function reload(img, base) {
    var sep = base.indexOf("?") === -1 ? "?" : "&";
    img.src = base + sep + "_r=" + Date.now();
  }

  function scheduleRetries(img) {
    if (!img || (tracked && tracked.has(img))) return;
    if (tracked) tracked.set(img, true);
    var base = (img.getAttribute("src") || "").split(/[&?]_r=/)[0];
    if (!base) return;

    RETRY_DELAYS.forEach(function (delay) {
      setTimeout(function () {
        if (!document.body.contains(img)) return;
        // Already loaded a real (wide) screenshot ? nothing left to fix.
        if (img.complete && img.naturalWidth && !isPlaceholder(img)) return;
        reload(img, base);
      }, delay);
    });
  }
  ...
  function scan(root) {
    if (!root) return;
    if (root.nodeType === 1 && root.matches && root.matches('img[src*="mshots"]')) {
      scheduleRetries(root);
    }
    if (root.querySelectorAll) {
      root.querySelectorAll('img[src*="mshots"]').forEach(scheduleRetries);
    }
  }
