...
(function(){
  ...
  /* WordPress mshots (used for "web" category screenshots) generates
     the screenshot asynchronously: the very first request(s) can come
     back as a 200 OK "still generating" placeholder instead of erroring,
     so a plain onerror handler never fires and the thumb/slide just
     looks blank. Fix: for any mshots URL, silently re-fetch it a few
     times with a cache-busting param as the real screenshot becomes
     ready, swapping it in once it does - harmless no-op for every
     other image host. */
  function isMshotUrl(u){
    return typeof u === "string" && u.indexOf("s.wordpress.com/mshots/") !== -1;
  }
  function scheduleMshotRefresh(img, baseSrc){
    if(!isMshotUrl(baseSrc)) return;
    [3000, 6000, 10000, 16000, 24000, 34000, 46000, 60000, 90000, 120000].forEach(function(delay){
      setTimeout(function(){
  ...
  /* Also expose globally so the Portfolio section's own "web" category
     thumbnails (built earlier in this file, before this IIFE runs) can
     reuse the exact same fix instead of duplicating the logic. */
  window.__scheduleMshotRefresh = scheduleMshotRefresh;
  ...
  CAT_ORDER.forEach(function(cat){
    ...
    state.slideEls = state.items.map(function(item, idx){
      ...
      if(item.type === "video"){
        ...
        scheduleMshotRefresh(media, item.src);
